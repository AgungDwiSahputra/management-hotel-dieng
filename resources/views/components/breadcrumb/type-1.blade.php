@props([
    'breadcrumbs' => [
        ...
        ['label' => 'Detail Reservation', 'url' => null], // null jika halaman aktif
    ],
])
    ...
    <nav>
        <ol class="flex items-center gap-1.5">
            @foreach ($breadcrumbs as $key => $breadcrumb)
                <li>
                    @if ($breadcrumb['url'])
                        <a class="inline-flex items-center gap-1.5 text-sm text-gray-500 dark:text-gray-400" href="{{ $breadcrumb['url'] }}">
                            {{ $breadcrumb['label'] }}
